...
void uprintf(char *fmt, ...) {
    ...
    HAL_UART_Transmit(&CMD_USART,(uint8_t *)print_buffer,size,1000);    //这一行是zx修改的
    //HAL_UART_Transmit_DMA(&CMD_USART, (uint8_t *)print_buffer, size); //这一行是老林框架中的
    /*
...
void send_wave(float arg1, float arg2, float arg3, float arg4) {
    ...
    HAL_UART_Transmit(&CMD_USART,(uint8_t *)s,22,1000);     //zx版本    
    //HAL_UART_Transmit_DMA(&CMD_USART, (uint8_t *)s, 22);  //老林版本
    //HAL_UART_Transmit(&huart1,(uint8_t *)s,sizeof(s),2000);
    //HAL_UART_Transmit_IT(&CMD_USART, (uint8_t *)s, 22);
}
